...
function Component () {
  return (
    <Article.T title={Title} subtitle={Subtitle} path={Path} postedAt={PostedAt}>
      ...
      <Article.P>
        <b>2.</b> <a href="https://buildwithtract.com/" target="_blank" rel="noreferrer">A Postmortem of a Startup</a><Article.BR />
        A detailed account of a tech startup’s failure to get off the ground. The co-founders provide a detailed account of what you literally do to run a company/get funded along with an analysis of their failures.<Article.BR />
        <Article.BR />
      ...
      <Article.P>
        <b>3.</b> <a href="https://gwern.net/improvement" target="_blank" rel="noreferrer">Everyday life improvements since the 90s</a><Article.BR />
      ...
      <Article.P>
        <b>4.</b> <a href="https://koomen.dev/essays/horseless-carriages/" target="_blank" rel="noreferrer">AI Horseless Carriage</a><Article.BR />
        A good model for why AI integrations feel so utterly useless. A particularly lackluster example that we’ve all experienced is Google's Gemini assistant which is effectively Clippy (no I don’t want Gemini to summarize a two sentence email or make meaningless statements about my spreadsheet!).<Article.BR />
        <Article.BR />
        The author even outlines a model for how “AI-first” products could actually be useful. It boils down to exposing the AI’s prompt + letting the AI take actions on the end-user’s behalf. I don’t think this totally nails it, but it’s certainly a step forward (and it allows me to avoid products with AI integrations completely).<Article.BR />
      </Article.P>

      <Article.P>
        <b>5.</b> <a href="https://www.wheresyoured.at/openai-is-a-systemic-risk-to-the-tech-industry-2/" target="_blank" rel="noreferrer">OpenAI Is A Systematic Risk To The Tech Industry</a><Article.BR />
        Turns out OpenAI has complex contractual agreements for new datacenter capacity + is losing lots of money. It is unclear if SoftBank will be able to fulfill their $30-40 billion cash injection this year, and whether OpenAI will need another $30-40 billion next year. Combined with the huge costs of AI inference, and the seemingly diminishing returns on larger AI models presents a real risk that OpenAI won’t exist in its current form in the next couple years.<Article.BR />
        <Article.BR />
        Whether OpenAI is alone “systematic” to the tech industry or whether the AI bubble as a whole remains to be seen.
      </Article.P>

      <Article.P>
        <b>6.</b> <a href="https://pages.cs.wisc.edu/~remzi/Naur.pdf" target="_blank" rel="noreferrer">Programming as Theory Building</a><Article.BR />
        A really great paper from the 1980s describing the outputs of programming as both the literal code produced, and the intangible mental model of the code held by the developers. In the SaaS world, that is the ability to deploy/resolve incidents quickly/fix bugs/add features/etc integrated over time.<Article.BR />
        <Article.BR />
